...
class Buyer(Person):
    ...
    # Gets all current attributes inputted at Menu.py, puts it into an array and writes the array into file
    def putintofile(self):
        arrayobj = [self.email_address, self.name, self.password, self.address, self.phonenum,self.shipping_address,self.account_type]
        with open("buyer_db.txt","a") as file:
...
class Seller(Person):
    ...
    # Gets all current attributes inputted at Menu.py, puts it into an array and writes the array into file
    def putintofile(self):
        arrayobj = [self.email_address, self.name, self.password, self.address, self.phonenum,self.bankaccnum,self.routnum,self.account_type]
        with open("seller_db.txt","a") as file:
            for line in arrayobj:
                file.write(("".join(line)+" "))
            file.write('\n')
